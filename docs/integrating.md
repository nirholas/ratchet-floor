# Integrating RatchetFloor

## From Solidity

```solidity
import {RatchetFloorHook} from "ratchet-floor/src/hooks/RatchetFloorHook.sol";
...
hook.configure(key, RatchetFloorHook.Config({ /* see the README */ }));
...
const address = hookAddress("ratchet-floor", 8453);              // Base
...
```solidity
IHookMetadata(hookAddress).hookName();   // "RatchetFloor"
...
More at https://ratchet-floor.pages.dev.
